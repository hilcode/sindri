...
/// A shell command line for a task. Lives here, alongside [`CommandOutput`], so [`crate::runtime`]
/// can accept it directly without the executor stringifying it first.
#[derive(Clone, Debug)]
pub struct ShellCommand(SmolStr);

impl ShellCommand {
    pub fn new(command: impl Into<SmolStr>) -> ShellCommand {
        ShellCommand(command.into())
    }
}
