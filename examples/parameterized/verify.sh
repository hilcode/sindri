...
debug_count=$(find .target/go-compile -mindepth 1 -maxdepth 1 -type d | wc -l)
[ "$debug_count" -eq 1 ] || fail "expected exactly one binding directory after the first build, got $debug_count"

echo "==> switching to mode = release and compiling again"
sed -i 's/mode = "debug"/mode = "release"/' "$build_file"
sindri compile
both_count=$(find .target/go-compile -mindepth 1 -maxdepth 1 -type d | wc -l)
[ "$both_count" -eq 2 ] || fail "expected both binding directories to coexist, got $both_count"

echo "OK: the debug and release bindings coexist side by side under go-compile/"
